exports.run = async (client, message, args) => {
  ...
  switch (args[0]) {
    case 'add':
      ...
      if (role == null) return message.reply(roleErr[Math.floor((Math.random() * roleErr.length))]);
    ...
    case 'remove':
      ...
      if (!roleName) return message.reply(roleErr[Math.floor((Math.random() * roleErr.length))]); //I need roleName
      if (role == null) return message.reply(roleErr[Math.floor((Math.random() * roleErr.length))]);
      if (!user.roles.find('name', roleName)) return message.reply(alreadyNo[Math.floor((Math.random() * alreadyNo.length))]);
    ...
    default:
      message.channel.send({
        embed: {
          "title": 'You should use that like \`role <add>|<remove> <user> <role>\`',
          "color": 0xff2222
        }
      });
  }
