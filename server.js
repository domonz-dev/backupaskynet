...
const db = require('quick.db'); //SQL database lib
...
client.on('message', async message => { //If recieves message
    ...
    let fetched = await db.fetch(`guild_${message.guild.id}`)
    if (fetched == null) {
    ...
    var prefix = fetched.prefix;
    var del = fetched.del;

    client.user.setPresence({
        game: {
            name: `на свой пепсис | #help`,
            type: 3
        }
    });
    ...
    if (message.content.toLowerCase() === 'а' || message.content.toLowerCase() === 'a' && message.channel.id === '464449977003802625') {
